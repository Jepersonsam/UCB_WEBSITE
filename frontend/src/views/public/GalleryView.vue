<template>
  <div
    class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8 sm:space-y-12 pt-12 sm:pt-16 lg:pt-20 pb-8"
  >
    <!-- Header Section -->
    <div class="text-center" data-aos="fade-down">
      ...
      <p class="mt-3 sm:mt-4 text-base sm:text-lg text-slate-600">
        Momen-momen indah dalam pelayanan dan kegiatan Unceasing Cantica Bandung
      </p>
      <div class="mt-4 sm:mt-6 flex justify-center">
        <div
          class="h-1.5 w-24 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-600"
        ></div>
      </div>
    </div>
    ...
    >
      <div
        class="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-indigo-100"
      >
        <svg
    ...
    <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      >
        ...
        >
          ...
          >
            <p class="text-sm font-semibold text-slate-200 line-clamp-2">
              {{ image.caption }}
            </p>
          </div>
        </div>
        <div
          class="absolute top-4 right-4 rounded-full bg-white/90 p-2 opacity-0 shadow-lg transition-opacity duration-300 group-hover:opacity-100"
        >
          <svg
    ...
    >
      ...
      <div class="relative max-h-[90vh] max-w-7xl">
        ...
        <div
          ...
          class="absolute bottom-0 left-0 right-0 rounded-b-2xl bg-gradient-to-t from-black/80 to-transparent p-6"
        >
          <p class="text-sm text-slate-200">{{ selectedImage.caption }}</p>
        </div>
      </div>
      <button
...
const fetchGallery = async () => {
  ...
  try {
    ...
    // Pastikan all_media adalah array
    const allMedia = responseData?.all_media || [];
    galleryImages.value = Array.isArray(allMedia) ? allMedia : [];

    console.log("Fetched gallery images:", galleryImages.value.length);
  } catch (error) {
...
const openLightbox = (image: GalleryImage) => {
  ...
  currentImageIndex.value = galleryImages.value.findIndex(
    (img) => img.id === image.id,
  );
