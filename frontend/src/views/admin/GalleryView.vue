...
const fetchGallery = async () => {
  ...
  try {
    ...
    // Pastikan albums dan allMedia adalah array
    albums.value = Array.isArray(responseData?.albums) ? responseData.albums : []
    allMedia.value = Array.isArray(responseData?.all_media) ? responseData.all_media : []
    
    console.log('Fetched albums:', albums.value.length)
    console.log('Fetched media:', allMedia.value.length)
  } catch (error) {
...
const deleteAllMedia = async () => {
  const result = await Swal.fire({
    ...
    text: `Anda akan menghapus ${allMedia.value.length} foto. Tindakan ini tidak dapat dibatalkan!`,
    icon: 'warning',
  ...
  try {
    const response = await api.delete('/v1/gallery/media')
    await fetchGallery()
    await swal.success(
      `Semua foto berhasil dihapus! (${response.data.deleted_count || allMedia.value.length} foto)`
    )
  } catch (error: any) {
    await swal.error(error.response?.data?.message || 'Gagal menghapus semua foto')
