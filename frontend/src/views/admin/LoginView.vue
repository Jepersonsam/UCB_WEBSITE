...
const handleLogin = async () => {
  ...
  try {
    ...
    // Jika response.data adalah string, parse manual
    if (typeof responseData === 'string') {
      // Hapus karakter 'x' di awal jika ada
      ...
      try {
        ...
        error.value = 'Error parsing response dari server.'
        return
      }
    }
    ...
    // Response structure dari backend (dari Network tab):
    // { message: 'Login successful', data: { user: {...}, token: '...' } }
    // atau
    // { status: 'success', message: 'Login successful', data: { user: {...}, token: '...' } }
    
    // Ambil token dan user - struktur: responseData.data.token dan responseData.data.user
    ...
    if (token && user) {
      ...
      console.error('❌ ERROR: Token or user not found')
      console.error('Response structure:', JSON.stringify(responseData, null, 2))
      console.error('Token value:', token)
      console.error('User value:', user)
      error.value = 'Response tidak valid. Token atau data user tidak ditemukan. Cek console (F12) untuk detail.'
    }
  } catch (err: any) {
