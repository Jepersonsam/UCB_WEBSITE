...
class MemberController extends Controller
{
    public function index()
    {
        ...
        $members->transform(function ($member) {
            ...
            if ($member->avatar) {
                // Return relative path, let frontend construct full URL
                $memberData['avatar_url'] = '/storage/' . $member->avatar;
            }
            return $memberData;
        });
    ...
    {
        // Validate first (without avatar path, since it's a file)
        ...
        // Handle avatar upload BEFORE creating member
        if ($request->hasFile('avatar')) {
            $avatarPath = $request->file('avatar')->store('members', 'public');
            $validated['avatar'] = $avatarPath;
            
            // Log avatar upload
            \Log::info('Member Store - Avatar Uploaded', [
                'avatar_path' => $avatarPath,
                'file_exists' => Storage::disk('public')->exists($avatarPath),
                'file_size' => Storage::disk('public')->size($avatarPath),
            ]);
        } else {
        ...
        // Log before creation
        \Log::info('Member Store - Before Create', [
            'validated_data' => $validated,
            'has_avatar' => isset($validated['avatar']),
        ...
        if ($member->avatar) {
            // Return relative path, let frontend construct full URL
            // Frontend will use backend base URL to access the file
            $memberData['avatar_url'] = '/storage/' . $member->avatar;
        }
